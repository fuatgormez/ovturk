...
class Order extends CI_Controller
{
    ...
    {
        // $getAll = $this->Model_order->export_order_data_all();

        // foreach($getAll as $row){
        //     $check = filter_var($row['billing_email'], FILTER_VALIDATE_EMAIL);
        //         if($check){
        //             echo strtolower( $check ) .'<br>';
        //         }
        // }
        // exit;
        ...
        if($this->input->post()){
            $getAll = $this->Model_order->export_order_data($this->input->post('status_id'), $this->input->post('store_id'), $this->session->userdata('land_id'));
